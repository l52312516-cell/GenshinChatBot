...
$output = Join-Path $root ".cache\tests\ocr_file_probe.exe"
& $zig cc -target x86_64-windows-gnu -std=c17 -DUNICODE -D_UNICODE -D_CRT_SECURE_NO_WARNINGS `
    tests\ocr_file_probe.c src\ocr.c src\vision.c src\capture.c src\config.c src\util.c third_party\cjson\cJSON.c `
    -Isrc -Ithird_party\cjson -o $output -municode -static `
    -luser32 -lgdi32 -ldwmapi -ld3d11 -ldxgi -lshlwapi -lwinhttp -lbcrypt -lole32 -lwindowscodecs -lm
